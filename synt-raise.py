...
class WaveletFaceDataset(Dataset):
    def __init__(self, transform=None, wavelet='db1'):
        ...
        real_dir = '/content/data/raise1k/RAISE_1k'
        fake_dir = '/content/data/synthbuster/synthbuster'

        # Not: max_images_per_class_* tanımlı ama kullanılmıyor; istersen slice ile sınırlayabilirsin.
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            return None
        wv = wavelet_transform_grayscale(img, self.wavelet)
        if self.transform:
...
class HybridModel(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.cnn = timm.create_model(
            'efficientnet_b1', pretrained=True, in_chans=3, num_classes=0, global_pool=''
        )
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=1280, nhead=8, dim_feedforward=2048, dropout=0.1, activation='relu'
        )
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=6)
        self.pos_embed = nn.Parameter(torch.zeros(1, 49, 1280))  # 7x7 patch = 49 token
...
if __name__ == "__main__":
    ...
    train_val_ds, test_ds = random_split(dataset, [n_train_val, n_test])
    ...
    # --------------------------- EARLY STOPPING ---------------------------
    # 15 epoch boyunca val_loss iyileşmezse durdur.
    patience   = 15
    min_delta  = 1e-4
    best_val   = float('inf')
    bad_epochs = 0
    ...
    for epoch in range(1, epochs + 1):
        ...
        if vl < best_val - min_delta:
            ...
            print(f"  [EARLY-STOP] New best val loss: {best_val:.6f} (ckpt saved)")
        else:
            bad_epochs += 1
            print(f"  [EARLY-STOP] No improvement ({bad_epochs}/{patience})")
    ...
    if os.path.exists(best_ckpt_path):
        ...
        print(f"[Main] Loaded best model from epoch {ckpt.get('epoch','?')} "
              f"with val_loss={ckpt.get('best_val_loss','?')}.")
